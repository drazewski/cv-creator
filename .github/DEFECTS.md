...
| 16 | Summary default section should be as a text not points | ✅ Fixed | Default Summary now seeds as a single plain-text block, and legacy untouched defaults migrate to the new format automatically |
| 17 | When editing text section (not list items) edited area (textarea) should expand the height to fit content | ✅ Fixed | Multiline editors now auto-grow to fit their content, including Summary text items in the US layout |